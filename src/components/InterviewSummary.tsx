...
export const InterviewSummary: React.FC<InterviewSummaryProps> = ({ session, onRestart }) => {
  ...
  const averageScore = session.questions.length > 0 
    ? Math.round(session.questions.reduce((sum, q) => sum + q.score, 0) / session.questions.length)
    : 0;
  
  const averageResponseTime = session.questions.length > 0
    ? Math.round(session.questions.reduce((sum, q) => sum + q.responseTime, 0) / session.questions.length)
    : 0;
      ...
      <div className="bg-white/70 backdrop-blur-sm rounded-3xl shadow-xl border border-white/50 p-8">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-600 text-lg">
            Great job completing your physics interview. Here's your performance summary:
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          ...
          <div className="p-6 bg-blue-50 rounded-2xl text-center">
            ...
            <div className="text-3xl font-bold text-blue-800">{averageScore}/10</div>
            <div className="text-sm text-gray-600">Average Score</div>
          </div>
          
          <div className="p-6 bg-purple-50 rounded-2xl text-center">
            <Clock className="w-8 h-8 text-purple-600 mx-auto mb-3" />
            <div className="text-3xl font-bold text-purple-800">{totalTime}m</div>
            <div className="text-sm text-gray-600">Total Time</div>
          </div>
          
          <div className="p-6 bg-indigo-50 rounded-2xl text-center">
            <Clock className="w-8 h-8 text-indigo-600 mx-auto mb-3" />
            <div className="text-3xl font-bold text-indigo-800">{averageResponseTime}s</div>
        ...
        {/* Detailed Results */}
        <div className="mb-8">
          <h3 className="text-xl font-semibold text-gray-800 mb-4">Question-by-Question Results</h3>
              ...
              <div key={index} className="p-4 bg-gray-50 rounded-xl">
                ...
                <div className="space-y-2 text-sm">
                  ...
                  <div>
                    ...
                    <span className="text-gray-600">{response.userAnswer}</span>
                  </div>
                  <div>
                    <span className="font-medium text-gray-700">Feedback: </span>
                    <span className="text-gray-600">{response.aiEvaluation}</span>
                  </div>
